...
//   document.querySelector(".time-calculator-output").textContent =
//     parts.join(", ");
// }
...
footballBox.addEventListener("click",(e)=>{
  const targetX = e.offsetX - footballBall.offsetWidth / 2;
  const targetY = e.offsetY - footballBall.offsetHeight / 2;

  const currentX = footballBall.offsetLeft;
  const currentY = footballBall.offsetTop;
  ...
  footballBall.style.left = targetX + "px";
  footballBall.style.top = targetY + "px";
  footballBall.style.transform = "rotate(" + rotation + "deg)";
});
